...
在 MCP 客户端中描述想画的内容，AI 就可以创建画布、查询颜色、放置像素或方形笔刷，查看返回的预览，再继续绘制并保存。

- **精确像素绘制：** 画布宽高均支持 1 至 128 像素，通过坐标控制每个落点。
- **固定色表：** 支持 24、144、221 色 MARD 色表，可以使用色号或当前色表的中文颜色名。
- **多尺寸笔刷：** 支持 1×1、2×2、4×4、8×8 方形笔刷，同一张画布可以切换笔刷并多次增量绘制。
...
这会绘制四个色块，输出一张 64×64 的 PNG。工具返回文字和 PNG 图片内容，需要支持图片工具结果的客户端才能直接预览。

## 选择导出类型

`save_image.output` 有三个选项，省略时仍只输出像素画，兼容已有调用：

| output | 保存内容 | filename 为 `cat.png` 时 |
| --- | --- | --- |
| `pixel`（默认） | 8 倍像素画 PNG；空格透明 | `cat.png` |
| `pattern` | 拼豆图纸 PNG | `cat.png` |
| `both` | 同一快照的像素画和图纸，按这个顺序返回两张图片 | `cat.png`、`cat-pattern.png` |

图纸每格写 MARD 色号，附从 1 开始的行列编号和每 5 格加粗线。下方矩形色块中只写色号和数量（例如 `H2 12`），不加颜色名称、分隔点或单位。图纸使用内置字体，不依赖 Python、系统字体或联网；当前不提供 SVG/PDF 或自动拆页，也不是实物 1:1 定位纸。画布坐标仍从 0 开始。
...
`background` 默认为 `white`（整张铺 H2 白豆）；设成 `empty` 时不放豆。`draw_pixels` 的 `-` 表示清除，支持相同的笔刷与整批校验。H2 始终是需要放的白色豆，空格不标号、不计数；上例只需 2 颗豆。白色区域不会自动当作背景删除。

两种导出共用规范化色号和占用信息，不从 RGB 反查色号。重叠绘制只统计最后留下的豆子。

每个文件单独原子保存；`both` 预检两个文件名，如已有文件则不写入。多个文件并非整体事务，磁盘故障或并发写入导致中途失败时，错误会列出本次已经保存的文件。省略文件名时整组重名会按数字后缀重试。`both` 的衍生文件名也必须满足 200 字节限制，超长会在写文件前报错。

## 工具与约束

| 工具 | 功能 |
| --- | --- |
| `create_canvas` | 创建画布（background: white 默认白豆 / empty 空格），宽高 1 至 128；锁定 24、144 或默认的 221 色表。成功重建会替换旧画布。 |
| `list_colors` | 查询当前锁定的色表；尚未创建画布时可以指定要查询的色表。 |
| `draw_pixels` | 接受空白分隔的 `x y 颜色` 三元组，笔刷为 1、2、4 或 8。 |
| `save_image` | output: pixel（默认像素画）、pattern（拼豆图纸）、both（两者），均为 PNG，不覆盖文件。 |

- 左上角为 `(0,0)`，向右为 X 正方向，向下为 Y 正方向。
- 笔刷坐标必须是笔刷尺寸的倍数，整个方块都必须在画布内。
- 重复落点按输入顺序覆盖；非法批次不会修改画布。
- 每次绘制最多接受 1 MiB 字符串和 65,536 个落点。
- 每个服务实例只有一张内存画布；重启会丢失未保存内容。暂不支持撤销、导入、会话持久化或原始尺寸导出。
- 像素画预览通过最近邻采样放大 8 倍；拼豆图纸使用独立的网格布局。24 色旧版色表的部分名称和 RGB 值与大色表不同，不应跨色表假定颜色一致。
